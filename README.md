...
EventBridge gives you a clean, scalable, **multiplayer‑ready** event‑driven architecture for Godot 4.  
Perfect for single‑player **and** multiplayer projects where raw signals/RPCs get messy.

- ✅ **Autogenerated API** – No more manual signal connections
- ✅ **Multiplayer‑Ready** – `to_server`, `to_all`, `to_id`, `emit_local`
- ✅ **Editor Integration** – Visual Event Registry & 1‑click API generation (EventDock)
...
- **Editor Integration** – Manage events visually with the EventDock panel.  
- **Duplicate Protection** – No more double signal connections.  
- **Persistent Caching** – Fast lookups even for large projects.  
- **One‑Line Cleanup** – `off_all()` removes dangling handlers.
...
If you’re using the demo project, open the EventDock and press **Generate** to create the initial registry and wrappers.
...
> Prefer **server‑authoritative** flows. Clients **request** (`to_server`), server **validates** and **broadcasts** (`to_all` / `to_id`).  
> Add per‑event validators like `func validate_request_roll(args: Array) -> bool:` for extra safety.
...
## 💖 Enjoy using EventBridge? Please support development

If you enjoy using EventBridge, consider supporting my work on Ko‑Fi or buying on itch.io.
...
- **Namespace‑based bus:** Groups events (e.g., `Game`, `Lobby`) and emits signals.  
- **Generated façade:** `EventManager.gd` helpers like `dice_broadcast(pid, result)`, `on_dice_broadcast(cb)`, `off_dice_broadcast(cb)`.  
- **Per‑event RPC config:** Authority (`authority`/`any_peer`), sync (`call_remote`/`call_local`), transfer mode (`reliable` / `unreliable` / `unreliable_ordered`), and channel → mapped at dispatch.  
- **Validation hooks:** Central/per‑event validators run before dispatch.  
...
- **Low‑level edge cases:** Custom binary streams or bespoke reliability semantics still belong to raw RPC/peers.
...
EventBridge adds light overhead (lookups, signal emission, validation). The heavy work (serialization/delivery) is still Godot’s networking, so performance is dominated by transport and payload size.

---

**TL;DR**: EventBridge is a structured, safer abstraction over RPC, not a replacement. It keeps gameplay code clean while relying on Godot’s networking stack.
